...
const Article = ({ newImg, numberValue, price, inf, about, content }) => {
    ...
    return (
        <div className='h-full flex items-center w-10/12 responsive relative'>
            <article 
                className='h-4/6 w-full flex justify-around items-center border-x border-[#ffffff57]'>
                {content && (
                    <>
                        <div className={`smartphone w-4/12 flex flex-col justify-cenyter ${!advanced ? 'items-center' : 'items-start'}`}>
                            {advanced && (
                                <>
                            ...
                            {!advanced && (
                                <div className="text-center">
                                    <div className="h-6 flex justify-center items-end">
                                        ...
                                        <p className="text-red-400 font-bold "><CgStopwatch className="w-6 h-6"/></p>
                                    </div>
                                    <img src={newImg} className="smartphone w-full " alt="Produto" />
                                    <p className="text-[#ffffff57] text-sm">{inf}</p>
                                    <p className="text-[#565656be] line-through">R$ 500,00</p>
                                    <p className="text-[#ffffff57] text-sm">{price}</p>
                                    <span className="cursor-pointer text-[#ec0808f6] text-sm py-2 text-decoration-underline" onClick={voltar}>voltar</span>
                                </div>
                            )}

                            <button
                                onClick={!advanced ? null : comprar}
                                className="
                        ...
                        {!hiddenImg && (
                            <img src={newImg} className="smartphone w-3/12 h-3/6 prodFilter" alt="Produto" />
                        )}
                    </>
                )}
            </article>
            <p className="absolute bottom-10 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-lime-500 text-2xl ftn ">
